...
function colorTable(){
	...
	for(var j=0;j<3;j++){
		...
		table += '<td ><table bgcolor="#000000"  border="0"  cellpadding="0" cellspacing="0"  class="color_table">';
		for(var i=0;i<12;i++){
			var clr ='#000000';
			if(j==1){
				clr = clrfix[i];	
			...
			if (i == 6) {
				table += '<tr><td style="border-top: 2px solid #fff;" bgcolor="'+clr+'" class="cell_color" onmouseover="showClr('+"'"+clr+"'"+')" onclick="setClr('+"'"+clr+"'"+')"></td></tr>';
			} else {
	...
	for (var c = 0; c<6; c++) {
		...
		table = table+'<table border="0" cellpadding="0" cellspacing="0" class="color_table"> ';
		for (var j = 0; j<6; j++) {
			table +="<tr>";
			for (var i = 0; i<6; i++) {
