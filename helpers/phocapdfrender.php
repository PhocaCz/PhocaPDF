	...
	public static function renderPDF( $document = '', $staticData = array()) {
		...
		switch ($option) {
			...
			case 'com_phocacart':
				...
				$results 	= $dispatcher->trigger('onBeforeCreatePDFPhocaCart', array (&$content, $staticData));
		...
		switch ($option) {
			...
			case 'com_phocacart':
				...
				$results = $dispatcher->trigger('onBeforeDisplayPDFPhocaCart', array (&$pdf, &$content, &$document, $staticData));
